...
def get_credentials_from_vault(vault_credentials):
    ...
    logging.critical("Vault credential address is %s", str(vault_address))
    secret_path = vault_credentials.get('secretPath', '/v1/secret/data/cred')
    logging.critical("Vault credential secret path is %s", str(secret_path))
    vault_auth = vault_credentials.get('authPath', '/v1/auth/kubernetes/login')
    logging.critical("Vault credential kubernetes auth path is %s", str(vault_auth))
